...
def find_aspect_ratios(video: dict) -> None:
    ...
    if response.status_code in get_cms().success_responses:
        ...
        for rendition in renditions:
            if rendition.get('rendition_id') == 'default/video6000' or rendition.get('audio_only') == False:
        ...
        if source_h and source_w:
            ...
            with open('report3.csv', mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([video_id, source_w, source_h, rendition.get('encoding_rate')])
        else:
...
if __name__ == '__main__':
    with open('report3.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Video ID', 'Aspect Ratio X', 'Aspect Ratio Y'])  # Write the header
