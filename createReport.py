...
row_list = [ ('account_id','id','created_at','custom_fields','tags')]
...
def create_report(video: dict) -> None:
	...
	"""
	row = (get_value(video, *default_split(field, separator=':shortform', maxsplits=1)) for field in row_list[0])
	with data_lock:
		row_list.append(row)
		show_progress()
...
if __name__ == '__main__':
    with SimpleTimer():
        ...
        try:
            list_to_csv(row_list, 'report_basic.csv')
